...
def quicksort(vector, inicio, fin, criterio):
    ...
    while(primero < ultimo):
        while(_criterio(vector[primero], criterio) <= _criterio(vector[pivote], criterio) and primero <= ultimo):
            primero += 1
        while(_criterio(vector[ultimo], criterio) > _criterio(vector[pivote], criterio) and ultimo >= primero):
            ultimo -= 1
        if(primero < ultimo):
            vector[primero], vector[ultimo] = vector[ultimo], vector[primero]
    if(_criterio(vector[pivote], criterio) < _criterio(vector[primero], criterio)):
...
class Lista(object):
    ...
    def _criterio(self, dato, criterio):
        if(criterio == None):
            return dato
        else:
            return dato[criterio]
    ...
    def insertar(self, dato, criterio=None):
        ...
        elif(self._criterio(dato, criterio) < self._criterio(self.__elementos[0], criterio)):
            self.__elementos.insert(0, dato)
        else:
            pos = 0
            while(pos < len(self.__elementos) and self._criterio(dato, criterio)>=self._criterio(self.__elementos[pos], criterio)):
                pos +=1 
    ...
    def busqueda(self, buscado, criterio=None, clave=None, criterio_clave=None):
        ...
        while(primero <= ultimo and pos == -1):
            ...
            if(self._criterio(self.__elementos[medio], criterio) == buscado):
                pos = medio
            elif(self._criterio(self.__elementos[medio], criterio) > buscado):
                ultimo = medio -1
            else:
                primero = medio + 1

        if(pos != -1 and clave is not None and self.__elementos[pos][criterio_clave] != clave):
            while(self._criterio(self.__elementos[pos], criterio) == self._criterio(self.__elementos[pos-1], criterio)):
                pos -= 1
            
            while(self.__elementos[pos][criterio_clave] != clave and 
                self._criterio(self.__elementos[pos], criterio) == self._criterio(self.__elementos[pos+1], criterio)):
                pos += 1
